...
if [[ "$MODE" == docker ]]; then
  ...
  docker compose --env-file "$ENV_FILE" --profile "$PROFILE" exec -T app php artisan radiushub:doctor --strict
  docker compose --env-file "$ENV_FILE" --profile "$PROFILE" exec -T freeradius freeradius -XC
else
